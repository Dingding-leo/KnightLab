...
- Active session: start FEN, result-aware PGN, game mode, bot level, orientation, optional resolved human colour (`w`/`b`) and requested colour choice (White/Black/Random), optional time control, current clock, pre-move clock history and optional typed termination.
- Completed game: ID, timestamp, mode, optional bot level, result, result-aware PGN, final FEN, ply count, optional resolved human colour and requested colour choice for bot games, optional time control, final clock values, optional typed termination and optional reviewed/review-key linkage.
...
- Preferences: original synthesized move sounds enabled/disabled plus normalized Stockfish executable path, profile, Elo, skill, strength limit, move time, optional depth/nodes, MultiPV, threads and Hash.

Clock, termination and player-side fields are optional so older sessions restore as Unlimited, White-player bot games and legacy timeout records remain readable. A present player-side field must use the bounded wire values (`w`/`b` and White/Black/Random), otherwise the record is rejected before it reaches UI state. Preferences are schema-validated; old sound-only records receive default engine settings and malformed or out-of-range engine values return to bounded defaults. Desktop SQLite is authoritative and browser localStorage remains a bounded compatibility fallback. A queued premove is deliberately non-persisted: it belongs only to the active bot search and is cleared by pause, restart, completion, undo, load and hydration. Stockfish process state, probe status, interactive search output and unfinished review jobs remain ephemeral.
