...
The Review workspace can load the current game, PGN or FEN; move through every main-line ply; flip the read-only replay board; and request one to five candidate lines from a dedicated Stockfish runtime. **Explore this position** then opens an explicitly temporary branch anchored to that exact FEN. Within the branch, the same board accepts legal click/drag moves and an explicit promotion choice; its displayed FEN drives the ordinary ambient Stockfish request/cache/cancellation path. The branch has undo, reset and return controls, does not change the PGN main line or completed report, and is discarded by main-line navigation, import or a new full-review run. Its FEN and a self-contained branch PGN (after the first move) can be copied/downloaded locally. Desktop uses a native process and the website uses Stockfish 18 Lite WebAssembly in a Worker. Users can choose Quick, Balanced or Deep effort, view scores from White's or the side-to-move perspective, and inspect SAN principal variations, WDL, depth, nodes, NPS, elapsed time and engine identity.
